...
class BuyOneDrinkPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => BuyTicketsCubit(sl.get<ProductRepository>())..getTickets(),
      child: Scaffold(
        appBar: AppBar(title: const Text(Strings.buyOneDrinkPageTitle)),
        body: BlocBuilder<BuyTicketsCubit, BuyTicketsState>(
          builder: (context, state) {
            if (state is BuyTicketsFiltered) {
              ...
              final BuyTicketsCubit buyTicketsCubit = context.read<BuyTicketsCubit>();
